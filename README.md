...
**不想配环境？直接下载 Windows 可执行文件**：[NaviRAG v1.0.1 · NaviRAG-dist.zip（980 MB）](https://github.com/MaPleooo9/NaviRAG/releases/latest/download/NaviRAG-dist.zip)
> 上面的链接始终指向最新版本；要下特定版本，去 [Releases 列表](https://github.com/MaPleooo9/NaviRAG/releases) 挑。
...
1. **三层知识库**：
   ...
   - **L2** 逃课/轮椅打法（**人工整理**，64 条）——逃课打法天然不在 wiki 上，wiki 记录正规流程，「卡石头」「蹲大树引魔像」只存在于玩家社区
   - **L3** 常规打法（**人工整理**，30 条）——不卡 bug、不利用地形，靠操作和机制理解打赢

   L2 和 L3 必须分开：混在一层时，问「怎么逃课」会召回归常规打法，问「怎么打」又会召回归逃课，两边都不对。想学真本事的人和二周目赶路的人要的不是同一种答案。
2. **规则路由 + 混合检索**：纯向量检索在「短问句 vs 长文档」场景下会被内容词干扰（实测：问大树守卫，召回的是文本里带「蹲大树」的铃珠猎人打法）。解法是实体命中硬置顶 + 三路召回分层加权，而不是调 embedding。路由还能识别否定表达——「怎么打**不逃课**」里也含"逃课"二字，纯关键词会误判，这里做了专门的否定词表。实体匹配还挂了一张**别名表**（`data/elden_ring/aliases.json`，61 个 boss / 167 条俗称），「女武神」「血王」「碎星」这类社区叫法也能定位到正确条目；它是纯查询侧数据，加俗称不用重建索引。

## 特性

- **本地 GPU 推理**：qwen3:8b 在 RTX 4070 Laptop 实测 41 tok/s（直答首字 ~1s），250 字回答约 9 秒；qwen2.5:3b 可达 97 tok/s
- **官方术语表**：从游戏文件提取的官方中英对照（对齐 15,751 条，去重后 10,129 条参与检索），中文提问可注入英文名增强跨语言匹配，回答中英双语
...
- **单实例防连点**：Windows 命名互斥量做单实例检测，手快连点两次只会有一个窗口，第二次点击直接唤起已有窗口（`FindWindow` + `SW_RESTORE`）。不用锁文件是因为进程崩溃后锁文件会残留、得手动删，而互斥量由内核持有，进程一退立即释放；检测在 `QApplication` 之前完成，第二个进程连 Qt 都不初始化就退出
...
python scripts/build_glossary.py    # 官方术语表（对齐 15,751 条，去重后 10,129 条）
...
- [ ] L2 扩充至 100+ 条（当前 55）
- [ ] L3 扩充至 50 条（当前 30）
- [x] 评估：85 条人工标注问法（含「口语俗称」「描述提问」两个难度组），Hit@1 12.9%（纯向量）→ **94.1%**（线上方案），MRR 0.951；含严格判定口径，以及规则 / 向量 / 别名的三重消融（见 docs/eval.md）
- [x] PyInstaller 打包：exe 与数据分离，改攻略后双击「更新攻略.bat」重建索引，无需重装环境
- [ ] 补盲测集（他人出题）+ Reranker 对比实验
